...
function hasFinanceTemplateEvents(events = []) {
  const text = toArray(events).map(item => item.event || item.line || "").join(" ").toLowerCase();
  return /initial decision or offer|risk signal appears|numbers stop matching expectation|final loss|financial decision|transaction detail|numbers pehle safe|risk quietly build/.test(text);
}
...
function buildBeatsFromTimeline(topic = "", researchContext = {}, mode = "story_documentary") {
  ...
  if (
    ["horror_mystery", "investigation_documentary", "local_claim_mystery", "record_based_mystery"].includes(mode) &&
    hasFinanceTemplateEvents(events)
  ) {
    return fallback;
  }
